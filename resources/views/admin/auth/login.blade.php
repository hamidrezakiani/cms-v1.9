                    ...
                    <div>
                        <div class="text-center my-5">
                            @if (!empty(config('Site.icon_logo')) && !empty(config('Site.text_logo')))
                                <img src="{{ asset('storage/configuration-images/'.config('Site.icon_logo')) }}">
                                <img width="150px" src="{{ asset('storage/configuration-images/'.config('Site.text_logo')) }}">
                            @else
                                <img src="{{ asset('images/logo.png') }}">
                                <img width="150px" src="{{ asset('images/logo-text.png') }}">
                            @endif
                        </div>
                        ...
                        <div class="row no-gutters">
                            <div class="auth-form">
                                ...
                                <form method="POST" action="{{ route('admin.login') }}">
                                    ...
                                    <div class="form-group">
                                        <label for="login_password" class="mb-1"><strong>{{ __('passwords.password') }}</strong></label>
                                ...
                                @if (Route::has('register'))
                                    <div class="new-account mt-3">
                                        <p>{{ __("common.do_not_have_account") }} <a class="text-primary" href="{{ url('/register') }}">{{ __('common.sign_up') }}</a></p>
                                    </div>
                                @endif
                            </div>
                        </div>

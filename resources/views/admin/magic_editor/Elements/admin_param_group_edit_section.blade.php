					...
					@foreach ($params as $value) 
						...
						<div class="col-md-12 mb-2 {{ $divClass }} {{ $hideDependDiv }}">
							...
							@elseif($value['type'] == 'attach_image')
								...
								@else
									<img src="{{ asset('images/mapps/banner.jpg') }}" height="80">
								@endif
							...
							@elseif($value['type'] == 'attach_multiple_images')
								...
								@else
									<img src="{{ asset('images/mapps/banner.jpg') }}" width="200">
								@endif
							...
							@elseif($value['type'] == 'dropdown_multi')
								...
								@endphp
								<label for="{{ $inputId }}" class="control-label">{{ $fieldHeading }}</label>
								<select name="{{ $inputName }}" id="{{ $inputId }}" class="default-select form-control ParamGroupInput {{ $classes }}" multiple="multiple" {{ $extra_tag }} style="height: 110px;">
									@forelse($fieldOptions as $dropdownKey => $dropdownVal)

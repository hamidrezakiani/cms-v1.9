	...
	<div class="row page-titles mx-0 ">
		<div class="col-sm-6 p-0">
			<div class="welcome-text">
				...
				<span>{{ __('common.add_blog') }}</span>
			</div>
		</div>
		<div class="col-sm-6 p-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
			<ol class="breadcrumb">
				<li class="breadcrumb-item"><a href="{{ route('blog.admin.index') }}">{{ __('common.blogs') }}</a></li>
				<li class="breadcrumb-item active"><a href="javascript:void(0)">{{ __('common.add_blog') }}</a></li>
			</ol>
		</div>
	</div>
					...
					<div class="col-md-12">
						<div class="card">
							<div class="card-header">
								<h4 class="card-title">{{ __('common.add_blog') }}</h4>
							</div>
							<div class="card-body p-4">
								<div class="row">
									<div class="form-group col-md-12">
										<label for="BlogTitle">{{ __('common.title') }}</label>
										<input type="text" name="data[Blog][title]" class="form-control MakeSlug" id="BlogTitle" placeholder="{{ __('common.title') }}" value="{{ old('data.Blog.title') }}" rel="slug">
						...
						<div class="card accordion accordion-rounded-stylish accordion-bordered XDiscussion {{ $screenOption['Discussion']['visibility'] ? '' : 'd-none' }}" id="accordion-discussion">
							...
							<div class="accordion__body p-4 collapse show" id="with-discussion" data-bs-parent="#accordion-discussion">
								<div class="form-check mb-2">
									...
									<input type="checkbox" name="data[Blog][comment]" class="form-check-input" id="ContentComment" value="1" @checked(old('data.Blog.comment') == 1 || config('Discussion.comment_status'))>
							...
							<div class="accordion__body p-4 collapse show" id="with-seo" data-bs-parent="#accordion-seo">
								<div class="row">
									<div class="col-md-12 form-group">
										...
										<input type="text" name="data[BlogSeo][page_title]" class="form-control" id="ContentSeoBlogTitle" placeholder="{{ __('common.blog_title') }}" maxlength="255" value="{{ old('data.BlogSeo.page_title') }}">
									</div>
									<div class="form-group col-md-6">
										<label for="ContentSeoMetaKeywords">{{ __('common.keywords') }}</label>
										<input type="text" name="data[BlogSeo][meta_keywords]" class="form-control" id="ContentSeoMetaKeywords" placeholder="{{ __('common.enter_meta_keywords') }}" maxlength="255" value="{{ old('data.BlogSeo.meta_keywords') }}">
									</div>
									<div class="form-group col-md-6">
										<label for="ContentSeoMetaDescriptions">{{ __('common.descriptions') }}</label>
										<textarea name="data[BlogSeo][meta_descriptions]" class="form-control" id="ContentSeoMetaDescriptions" rows="5" placeholder="{{ __('common.enter_meta_descriptions') }}">{{ old('data.BlogSeo.meta_descriptions') }}</textarea>
									</div>
								</div>
							</div>
						</div>
								...
								<div class="row">
									<div class="col-md-12 form-group">
										...
										<select name="data[Blog][status]" id="Status" class="default-select form-control">
											...
											<option value="4" {{ old('data.Blog.status') == 4 ? 'selected="selected"' : '' }}>{{ __('common.private') }}</option>
											<option value="5" {{ old('data.Blog.status') == 5 ? 'selected="selected"' : '' }}>{{ __('common.pending') }}</option>
										</select>
									</div>
									<div class="col-md-12 form-group">
										...
										<select name="data[Blog][visibility]" id="ContentVisibility" class="default-select form-control">
											...
											<option value="Pr" {{ old('data.Blog.visibility') == 'Pr' ? 'selected="selected"' : '' }}>{{ __('common.private') }}</option>
										</select>
									</div>
					...
					<div class="col-md-12">
						<div class="card accordion accordion-rounded-stylish accordion-bordered XTags {{ $screenOption['Tags']['visibility'] ? '' : 'd-none' }}" id="accordion-tags">
							...
							<div class="accordion__body p-4 collapse show" id="with-tags" data-bs-parent="#accordion-tags">
								<input type="text" name="data[BlogTag]" data-role="tagsinput" class="form-control bootstrap-tagsinput" placeholder="{{ __('common.type_tags_here') }}" id="BlogBlogTag" value="{{ old('data.BlogTag') }}">
							</div>
						</div>
					</div>
					<div class="col-md-12">
					...
					<div class="col-md-12">
						<div class="card accordion accordion-rounded-stylish accordion-bordered XVideo {{ $screenOption['Video']['visibility'] ? '' : 'd-none' }}" id="accordion-video">
							...
							<div class="accordion__body p-4 collapse show" id="with-video" data-bs-parent="#accordion-video">
								...
								<input type="text" name="data[BlogMeta][1][value]" class="form-control bootstrap-tagsinput" placeholder="{{ __('common.youtube_video_link') }}" id="BlogMeta1Value" value="{{ old('data.BlogMeta.1.value') }}">
							</div>
						</div>
					</div>
				</div>

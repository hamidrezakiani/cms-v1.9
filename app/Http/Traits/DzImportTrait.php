...
trait DzImportTrait {
                ...
                foreach($blogXml->blog_categories->blog_categories as $blog_category) {
                    ...
                    if (!BlogCategory::whereSlug($blog_category->slug)->exists()) {
                        ...
                        if (isset($blog_category->image) && !empty($blog_category->image)) {
                            $category = BlogCategory::whereSlug($blog_category->slug)->update(['image' => $blog_category->image]);
                        }
            ...
            if (isset($blogXml->blog_categories->blog_categories)) {
                ...
                foreach($blogXml->blog_categories->blog_categories as $blog_category) {
                    ...
                    if (!W3cptBlogCategory::whereSlug($blog_category->slug)->exists()) {
                        ...
                        if (isset($blog_category->image) && !empty($blog_category->image)) {
                            $category = W3cptBlogCategory::whereSlug($blog_category->slug)->update(['image' => $blog_category->image]);
                        }
    ...
    public function exportData($request, $filename)
    {
        ...
        if ($request->content == 'all_content') {
            $content['w3cms']['blogs'] = Blog::where('post_type', '=', config('blog.post_type'))->with('blog_meta', 'blog_seo', 'blog_categories', 'blog_tags')->orderBy('created_at', 'asc')->get()->toArray();
            ...
            $content['w3cms']['w3cpt_blogs'] = W3cptBlog::where('post_type', '!=', config('blog.post_type'))->with('blog_meta', 'blog_seo', 'blog_categories', 'blog_tags')->orderBy('created_at', 'asc')->get()->toArray();
        }
